...
def factor_crop_rec(factor, factor_value,factor_normal,crop_name):
    ...
    model="gpt-4o-mini",
    messages = [
        {"role": "system", "content": "You are a recommender for rural farmers. only use block of text in 100 words strictly and make responses dynamic, The response should be in 2 paragraphs, first paragraph should speak to general practice and second paragraph should speak to rural alternative to help farmers(you could add solutions/resources native to Kenya, with Kenyan traditional names, if you add Kenyan traditional name, describe it briefly)"},
        {"role": "user", "content": f"""Provide Recommendation to improve/maintain the {factor} level of soil for planting {crop_name}.
                                            The soil has {factor_value} of {factor} and the average of {factor} for {crop_name} is {factor_normal} or within that range."""},
    ]
